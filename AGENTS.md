...
**AI Audio Starter Kit**. Read this first.

## 1. Repository Map

```
apps/web/                                  Next.js 16 frontend (App Router, Tailwind v4, shadcn/ui)
  src/app/library/                         /library — audio asset grid (sample-specific)
  src/app/upload/                          /upload — drag-and-drop audio upload
  src/app/files/                           /files  — full-bucket explorer (kept from starter)
  src/components/library/                  audio-asset-card, library-view, waveform
services/api/                              FastAPI backend (layered: types/config/repo/service/runtime)
  app/runtime/library.py                   /library, /library/{key}/playback|download, DELETE /library/{key}
  app/service/library.py                   audio key validation, list/head/delete orchestration
  app/service/audio_metadata.py            wave + mutagen extractor
  app/repo/b2_client.py                    boto3 — generic file helpers + audio Library helpers
  app/types/library.py                     AudioAsset
packages/shared/                           Shared TypeScript types (FileMetadata, AudioAsset)
