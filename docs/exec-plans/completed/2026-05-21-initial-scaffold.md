...
Scaffolded `ai-audio-starter-kit` from the `vibe-coding-starter-kit` template
...
- Identifier rename `vibe-coding-starter-kit` -> `ai-audio-starter-kit`
  across every text file in the tree (excluding `node_modules`, `.next`,
  `dist`, `build`, `.venv`).
- Workspace packages: `@ai-audio-starter-kit/{shared,web}`.
- Custom user agent set on the only S3 client to
  `b2ai-ai-audio-starter-kit`.
